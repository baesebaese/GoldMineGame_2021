...
def setup_gemstone():
    ...
    # 작은 금
    small_gold = Gemstone(gemstone_images[0], (200, 380),small_gold_price, small_gold_speed) # 0번째 이미지를 200, 300 위치에
...
def display_score():
    ...
    screen.blit(txt_goal_score, (50, 60))

def display_time(time):
    txt_time = game_font.render(f"Time: {time}", True, BLACK)
    screen.blit(txt_time, (1000, 50))

def display_game_over():
    game_font = pygame.font.SysFont("arialrounded", 60) # 큰 폰트
    txt_game_over = game_font.render(game_result, True, BLACK)
    rect_game_over = txt_game_over.get_rect(center=(int(screen_width / 2), int(screen_hight / 2)))
...
while running:
    ...
    if not caught_gemstone: # 잡힌 보석이 없다면 충돌 체크
        for gemstone in gemstone_group:
          #  if claw.rect.colliderect(gemstone.rect): # 직사강형 기준으로 충돌 처리
          if pygame.sprite.collide_mask(claw, gemstone) : # 투명 영역은 제외하고 실제 이미지 영역에 충돌 처리
